...
3. Add the fixes to commit history before pushing again. Do **not** land a standalone formatting-only or lint-only commit (see "What Not to Do" below); instead, either amend the commit(s) that introduced the failures, or include the fixes in your next substantive commit on the same branch. For `copilot/**` branches, the auto-fix workflow described under "Auto-Fix Workflow (Safety Net for Copilot Branches)" will normally apply these fixes automatically.
